...
public class TP02Q01 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for ( i = 1; i < 200; i++) {
            linha[i] = scanner.nextLine();
            String[] primeiro = linha[i].split(",");
            Scanner sclinha = new Scanner(Arrays.toString(primeiro));
            String idArqString = primeiro[i-1];
            System.out.println(Arrays.toString(primeiro));
            int idArq = Integer.parseInt(idArqString);
            while (isFim(idStr) == false) {
                if (idString == idArq) {

                    jogador = new Jogador();
                    jogador.Ler(sclinha, idArq);
                    jogador.Imprimir();
                    idStr = sc.nextLine();
